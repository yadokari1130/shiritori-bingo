...
export function buildCardCharPool(options: {
  ...
  if (options.yoon)
    pool.push(...YOON_CHARS)
  if (options.sokuon)
    pool.push(...SOKUON_CHARS)
  if (options.prolonged)
    pool.push(...PROLONGED_CHARS)
  if (options.smallA)
    pool.push(...SMALL_A_CHARS)
  if (options.dakuten)
    pool.push(...DAKUTEN_CHARS)
  if (options.handakuten)
    pool.push(...HANDAKUTEN_CHARS)
  return pool
}
...
export function normalizeTailChar(word: string): { char: string, valid: boolean } {
  ...
  if (i < 0)
    return { char: '', valid: false } // 「ーー」など

  const ch = normalized[i]

  // 1. 語尾が「ん」なら無効（「ん」「んー」など）
  if (ch === 'ん')
    return { char: 'ん', valid: false }

  // 2. 拗音 → 直音
  if (YOON_CHARS.includes(ch)) {
    if (!hasNormalCharBefore(normalized, i))
      return { char: '', valid: false }
...
export function validateWordRuleRequirements(
  ...
  const trimmed = word.normalize('NFC').trim()
  const { requiredStartChar, usedWords = [], minWordLength, maxWordLength } = options
